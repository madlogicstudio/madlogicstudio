...
function ShowcaseCard({ video, title, info }: ShowcaseProps) {
    ...
    return (
        <div className="h-[460px] w-[360px] flex flex-col items-start justify-start cursor-pointer
            ...
            <div className="flex flex-col items-start justify-start p-[calc(0.4vw+0.6rem)] gap-[calc(0.4vw+0.6rem)]">
                <span className="text-[var(--light-color)] spacemono text-[calc(0.4vw+0.9rem)] font-semibold">{title}</span>
                <span className="text-[var(--light-color)] spacemono text-[calc(0.4vw+0.7rem)]">{info}</span>
            </div>
        </div>
    )
}
